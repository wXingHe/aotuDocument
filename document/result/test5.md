...
## 6.12      将SQL语句中的__TABLE_NAME__字符串替换成带前缀的表名（小写）      
...
## 6.21      批处理执行SQL语句      批处理的指令都认为是execute操作      
